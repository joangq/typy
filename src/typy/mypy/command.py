...
def report(**kwargs: AnyDict):
    now = datetime.now()
    stdout, stderr, elapsed = _run(**kwargs)
    objects = stdout.strip().split('\n')
    ...
    try:
        ...
        __args = ' '.join(Mypy.from_dict(kwargs).build())
    ...
    for message in analysis.messages:
        ...
        severity = SEVERITY.get(message.severity, None)
        if not severity:
    ...
    return gitlab.Report(
        ...
        time=now,
        emitter=Report.Emitter(name='mypy', version=version())
    )